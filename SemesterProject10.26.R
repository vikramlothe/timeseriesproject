...
ts.plot(cbind(rnondurspend, rdurspend), col=c("black","red"),
        lty=c("solid", "solid"), main= "Real consumption, classified
by type", ylab = "")
...
ts.plot(cbind(inflation, unem/100), col=c("black", "red"),
        lty=c("solid", "solid"), main="US National inflation and unemployment,
1974-2023", ylab="Percent")
legend("topleft", bty = "n", legend=c("quarterly inflation (GDP deflator)",
                                      "unemployment rate"),
       fill=c("black", "red"))
...
cimpulse_d_rndch = irf(varmodel, impulse = "d_rgovch", response = "d_rndch",
                       n.ahead = 8, cumulative = TRUE, ortho = TRUE)
cimpulse_d_rdch = irf(varmodel, impulse = "d_rgovch", response = "d_rdch",
                      n.ahead = 8, cumulative = TRUE, ortho = TRUE)
...
plot(impulse_d_inf1)
plot(impulse_d_inf2)
plot(cimpulse_d_rndch)
plot(cimpulse_d_rdch)
...
#Glance at data from 2020-2024
d_unem=na.omit(diff(unem))
ts.plot(cbind(tail(rndch, 20), tail(rdch, 20)), 
        col=c("black", "brown"),lty=c("solid", "solid", "dotted"),
        main="Changes in consumption, 2019-2023",
        ylab= "% growth, quarterly")
legend("topright",
       bty = "n",
       legend=c("Non-durable consumpton (including services)",
                "Durable consumption"),
       fill=c("black", "brown", "blue"), cex=0.7)

ts.plot(tail(d_unem, 20),
        col="blue",
        main="Changes in unemployment rate, 2019-2023",
        ylab="Percentage change, quarterly")

ts.plot(cbind(window(rndch,start=c(2008,1), end=c(2012,4)),
              window(rdch,start=c(2008,1), end=c(2012,4))),
        col=c("black", "brown"),
        lty=c("solid", "solid", "dotted"),
        main="Changes in consumption, 2008-2012",
        ylab= "% growth, quarterly")
legend("bottomright",
       bty = "n",
       legend=c("Non-durable consumpton (including services)",
                "Durable consumption"),
       fill=c("black", "brown"), cex=0.7)

ts.plot(window(d_unem,start=c(2008,1), end=c(2012,4)),
        col="blue",
        main="Changes in unemployment rate, 2008-2012",
        ylab="Percentage change, quarterly")
